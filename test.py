...
# # app.exec()
#
# p = [(i, j) for i in range(154//7) for j in range(7)]
# print(len(p))
...
class StickerTextEdit(QTextEdit):
    def __init__(self):
        super().__init__()

    def insertSticker(self, image_path):
        cursor = self.textCursor()
        image = QImage(image_path)

        if not image.isNull():
            label = QLabel()
            label.setPixmap(image.scaled(32, 32, Qt.KeepAspectRatio))
            cursor.insertText(" ")  # Inserting space for the sticker
            cursor.insertHtml(f'<img src="{image_path}" width="32" height="32"> ')
            cursor.insertText(" ")  # Inserting space after the sticker
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        self.text_edit = StickerTextEdit()
        layout = QVBoxLayout()
        layout.addWidget(self.text_edit)

        self.setLayout(layout)
        self.setWindowTitle('Sticker in QTextEdit Example')
        self.resize(400, 300)

        # Example usage: insert sticker at cursor position
        self.text_edit.insertSticker('path/to/your/sticker.png')
